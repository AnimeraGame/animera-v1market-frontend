...
const CardsList = ({
	...
	const handleBuy = async item => {
		...
		if (result && result.status === true) {
			...
			updateEstateNftMutation.mutate(payload, {
				onSuccess: estate => {
					if (estate) {
						setData((data.filter(one => (one.id + '') !== (estate.updateEstateStatus.id + ''))))
		...
		} else if (result && result.code === 4001) {
			setIsSubmitting(false)
			setIsBuyModalOpen(false)
			setSelectedCard({})
			setPriceErrorMessage(t('somethingWentWrongPriceUpdate'))
													...
													menuItems={[
														{
															...
															onClick: () => {
																router.push(
																	`${router.asPath}${router?.asPath.length > 13 ? '&' : '?'
