...
const MarketplaceWrapper = ({ t }) => {
	...
	const {
		...
		[
			`FETCH_DIRECT_OFFERS_FIRST_${18}_AFTER_${afterId}_DIRECTION_${selectedOrder !== 'high_low' ? 'desc' : 'asc'
			}_FIELD_${selectedOrder}_QUERY_MIN_${filters.price[0]}_MAX_${filters.price[1]}_R_${filters.scarcity
			}_SEARCH_${searchText}`,
		],
		{
			onePage: 18,
